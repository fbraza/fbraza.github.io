...
- The predictions service which was deployed on the google cloud with on App Engines. One worker service that was processing the uploaded users nights to generate predictions and clinical metrics.
- And one poller service that was constantly pinged by a cron jobs to scan if new nights were uploaded in our data lake (Google Storage equivalent of the S3 buckets for the AWS aficionados).

The training infrastructure consisted of ... one computer with a graphic card.

Moreover labeled data that include nights recorded both by polysomnography and sunrise devices were stored kind of “everywhere”: on the cloud, on hard drives and on DropBox.

Collecting and processing this data was a long process. To just collect them, anonymize the file and exclude missing data, the original pipeline run locally was lasting between 30 seconds to maximum one and a half minute by file! With more than 2000 files to process it could take between one to three days.

And this is without having extracting any features! Just the data preparation! Training the model was taking a couple more day. So in general, it took a bit more than a week of work to process, train evaluate and re-iterate. In software engineering and more broadly in any active process, speed of iteration is critical. I am quite fan of the Boyd law of iteration:
...
Failing fast and being able to iterate fast is key. In the next sections I am going to unfold how we design and implement our machine learning training playground on Google Cloud. **How we scale the data pipeline and manage to iterate through the complete cycle in half a day instead of a week.**
